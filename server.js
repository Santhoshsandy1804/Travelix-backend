...
app.post("/api/upload/destination", (request, response) => {
    ...
    const sqlQuery = `insert into sandy_destinations (destinationName, destinationImage, destinationCount) values ('${data.destinationName}', '${data.destinationImage}', '${data.destinationCount}')`;
...
app.get("/api/list/destination", (request, response) => {
    ...
    if(destName == undefined){
        sqlQuery = "select * from sandy_destinations";
    }
    else{
        sqlQuery = `select * from sandy_destinations where destinationName='${destName}'`
    }
...
app.delete("/api/delete/destination/:id", (request, response) => {
    ...
    const sqlQuery =  `delete from sandy_destinations where id=${id}`;
...
app.post("/api/upload/hotel", (request, response) => {
    ...
    const sqlQuery = `insert into sandy_hotels (hotelName, hotelPrice, hotelLocation, hotelImage) values ('${data.hotelName}', '${data.hotelPrice}', '${data.hotelLocation}', '${data.hotelImage}')`;
...
app.get("/api/list/hotels", (request, response) => {
    ...
    if(hotelName == undefined){
        sqlQuery = "select * from sandy_hotels";
    }
    else{
        sqlQuery = `select * from sandy_hotels where hotelName='${hotelName}'`
    }
    connection.query(sqlQuery, (error, result) => {
        if(error){
            response.status(500).send(error);
...
app.delete("/api/delete/hotel/:id", (request, response) => {
    ...
    const sqlQuery = `delete from sandy_hotels where id=${id}`;
